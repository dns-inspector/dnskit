...
final class DNSClientTCPTests: XCTestCase, IClientTests {
    ...
    func testPTRQuery() async throws {
        try await ClientTests(transportType: .DNS, transportOptions: TransportOptions(dnsPrefersTcp: true), serverAddress: "1.1.1.1:53").testPTRQuery()
    }
    ...
    func testQueryUTF8() async throws {
        try await ClientTests(transportType: .DNS, transportOptions: TransportOptions(dnsPrefersTcp: true), serverAddress: "1.1.1.1:53").testQueryUTF8()
    }
    ...
    func testAuthenticateNXDOMAIN() async throws {
        try await ClientTests(transportType: .DNS, transportOptions: TransportOptions(dnsPrefersTcp: true), serverAddress: "1.1.1.1:53").testAuthenticateNXDOMAIN()
    }
