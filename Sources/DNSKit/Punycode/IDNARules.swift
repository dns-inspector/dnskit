...
internal struct IDNARules {
    public static func getRules(for codepoint: UInt32) -> [IDNARule] {
        var ruleCount = Int(0)
        // rule lists are split into pages based on the first 2 bytes of the code point, the `find_idna_rules` method returns
        // all rules for codepoints that have the same binary prefix.
